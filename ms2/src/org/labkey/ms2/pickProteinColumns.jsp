...
<table class="labkey-data-region"><tr>
  <td><form method="post" action=""><table class="labkey-data-region">
    <tr>
        ...
        <td nowrap><input type="image" src="<%=PageFlowUtil.buttonSrc("Pick")%>" onclick="setCurrent('<%=h(bean.commonColumns)%>');return false;"> <input type="image" src="<%=PageFlowUtil.buttonSrc("Add")%>" onclick="appendToCurrent('<%=h(bean.commonColumns)%>');return false;"></td>
    </tr>
    <tr>
        <td nowrap><strong>Protein Prophet:</strong></td>
        <td><%=h(bean.proteinProphetColumns)%></td>
        <td nowrap align=right><input type="image" src="<%=PageFlowUtil.buttonSrc("Add")%>" onclick="appendToCurrent('<%=h(bean.proteinProphetColumns)%>');return false;"></td>
    </tr>
    <tr>
        <td><strong>Quantitation:</strong></td>
        <td><%=h(bean.quantitationColumns)%></td>
        <td nowrap align=right><input type="image" src="<%=PageFlowUtil.buttonSrc("Add")%>" onclick="appendToCurrent('<%=h(bean.quantitationColumns)%>');return false;"></td>
    </tr>
    <tr>
        <td>&nbsp;</td>
    </tr>
    ...
    <tr>
        ...
        <td nowrap ><input type="image" src="<%=PageFlowUtil.buttonSrc("Pick")%>" onclick="setCurrent('<%=h(bean.defaultColumns)%>');return false;"> <input type="image" src="<%=PageFlowUtil.buttonSrc("Add")%>" onclick="appendToCurrent('<%=h(bean.defaultColumns)%>');return false;"></td>
    </tr>
      <tr>
          <td>&nbsp;</td>
      </tr>
