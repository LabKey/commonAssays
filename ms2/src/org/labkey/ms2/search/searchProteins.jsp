...
<labkey:form action="<%= new ActionURL(MS2Controller.DoProteinSearchAction.class, getContainer()) %>">
    <table class="lk-fields-table">
        ...
        <tr>
            ...
            <td colspan="100">
                <div><input type="radio" name="<%=h(PeptideFilteringFormElements.peptideFilterType)%>" value="<%= h(MS2Controller.ProphetFilterType.none.name()) %>" <%=checked(bean.getForm().isNoPeptideFilter())%> />None</div>
                <div style="padding-top: 5px"><input type="radio" name="<%=h(PeptideFilteringFormElements.peptideFilterType)%>" id="peptideProphetRadioButton" value="<%= h(MS2Controller.ProphetFilterType.probability.name()) %>" <%=checked(bean.getForm().isPeptideProphetFilter())%>/>Minimum PeptideProphet prob <input onfocus="document.getElementById('peptideProphetRadioButton').checked=true;" type="text" size="4" name="<%=h(PeptideFilteringFormElements.peptideProphetProbability)%>" value="<%=h(bean.getForm().getPeptideProphetProbability() == null ? "" : bean.getForm().getPeptideProphetProbability())%>" /></div>
                <div style="padding-top: 5px"><input type="radio" name="<%=h(PeptideFilteringFormElements.peptideFilterType)%>" id="customViewRadioButton" value="<%= h(MS2Controller.ProphetFilterType.customView.name()) %>"<%=checked(bean.getForm().isCustomViewPeptideFilter())%>/>Custom filter:
