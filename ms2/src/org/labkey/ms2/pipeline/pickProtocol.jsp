...
[<a href="<%=urlFor(PipelineController.ShowCreateMS2ProtocolAction.class, "path", form.getPath(), "searchEngine", form.getSearchEngine())%>">create a new protocol</a>]<%= PageFlowUtil.helpPopup("Create a new protocol", "Creating a new protocol lets you describe the steps taken to process a sample, or any special configuration for the mass spectrometer.") %>
&nbsp;&nbsp;&nbsp;[<a href="<%=getContainer().urlFor(PipelineController.SearchAction.class, "path", form.getPath(), "searchEngine", form.getSearchEngine(), "skipDescription", "true", "provider", "ms2", "action", "search")%>">skip this step and continue</a>]
<br>
<br>
<%
    if(form.hasErrors())
...
<table>
    <tr>
        ...
        <td>
            <labkey:button text="Cancel" href="<%=PipelineService.get().urlReferer(getContainer())%>"/>
        </td>
    </tr>
</table>
