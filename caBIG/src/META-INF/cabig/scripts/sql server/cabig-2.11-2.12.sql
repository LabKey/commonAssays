...
WHERE pgm.ProteinGroupId IN (SELECT pg.RowId FROM cabig.ProteinGroups pg)
GO
...
WHERE pm.ProteinGroupId IN (SELECT pg.RowId FROM cabig.ProteinGroups pg)
GO
...
WHERE s.Deleted = 0 
	AND s.SeqId IN (SELECT SeqId FROM cabig.FastaSequences)
GO
