...
import org.labkey.microarray.ArrayManager;
...
public class AgilentFeatureExtractionClientImpl implements FeatureExtractionClient {
    ...
    public AgilentFeatureExtractionClientImpl(String url, Logger instanceLogger) {
        _url = url;
        if(instanceLogger == null) _instanceLogger = Logger.getLogger(AgilentFeatureExtractionClientImpl.class);
        ...
        taskId = Long.toString(new Date().getTime());
    }
    ...
    public String testConnectivity() {
        ...
        if (0 == errorCode) {
            ...
            return (("".equals(errorString)) ? "Fail to contact Feature Extraction server at " + _url : errorString);
        }
    }
    ...
    public void findWorkableSettings() {
        ...
        try {
                ...
                if (!(1 == attemptStatus || 2 == attemptStatus)) {
                    if (0 == attemptStatus) {
                        if (!url.equals(testUrl))
                            errorString = "Test passed ONLY when feature extraction server is set to " + alternativeLink.toString();
                        ...
                        errorCode = attemptStatus;
                        errorString = "FeatureExtraction server responded on " + testUrl + " with \"" + attemptMessage + "\"";
            ...
            errorCode = 1;
            errorString = "Fail to parse FeatureExtraction Server URL";
        }
    }
    ...
    public int run(File[] imageFiles) {
        ...
        _instanceLogger.info("Submitting job to FeatureExtraction server (taskId=" + taskId + ").");
        if (!submitFiles(taskId, imageFiles)) {
            ...
            File resultsFile= ArrayPipelineManager.getResultsFile(imageFiles[0].getParentFile(),taskId);
            int retryCount = 0;
            while(retryCount++ < 3) {
                ...
                while (true) {
                    ...
                    extractionStatus = getTaskStatus(taskId);
                ...
                if (!extractionStatus.toLowerCase().contains("complete")) {
                    ...
                    if (getResultFile(taskId, resultsFile)) {
    ...
    public int saveProcessedRuns(User u, Container c, File outputDir) {
        ...
        for (File mage : mageFiles) {
            ...
            try {
                ...
                ArrayManager.saveFeatureExtractionRun(u, c, feRun);
                runs++;
            } catch (Exception e) {
    ...
    private Properties request(Properties parameters) {
        ...
        try {
            ...
            errorCode = 1;
            errorString = "Fail to parse FeatureExtraction Server URL: " + x.getMessage();
            results.setProperty("error", "1");
            results.setProperty("errorstring", errorString);
        } catch (Exception x) {
            _instanceLogger.error("Exception "+x.getClass()+" connect("+_url+"," + ")=" + feRequestURL);
            errorCode = 2;
            errorString = "Failed to interact with FeatureExtractionQueue application: " + x.getMessage();
